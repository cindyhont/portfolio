    ...
    Scene = (
        ...
        useFrame(({invalidate},delta)=>{
            ...
            meshes.forEach((m,i)=>{
                ...
                if (!mobile) material.uniforms.heightMap.value = gpuCompute.getCurrentRenderTarget( heightMapVariables[i] ).texture;
            })
            ...
            <div ref={canvasRef}>
                <Context.Consumer>{({devicePixelRatio})=>
                    <Canvas dpr={devicePixelRatio} style={{position:'absolute'}} frameloop='demand'>
                        <Scene imageIdList={imageIdList} start={start+200} end={end} />
                    </Canvas>
                }</Context.Consumer>
            </div>
        )
